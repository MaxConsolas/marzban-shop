  ...
  try {
    ...
    for (const marzbanUser of upcoming) {
      try {
        ...
        const expire = marzbanUser.expire;
        if (!expire) {
          continue;
        }
        const expirationLabel = expire < now + HOURS_12 ? translate(phrases.today) : translate(phrases.tomorrow);
        const message = translate(phrases.renewReminder, {
